...
const discountedPrice = computed<number | null>(() => {
  if (!hasValidDiscount.value) return null;

  const discount = props.product.discountPercentage as number;
...
<template>
  ...
  >
    <div class="relative bg-slate-100 aspect-[4/3] sm:aspect-[3/2]">
      ...
      <Badge
        v-if="product.stock > 0 && product.stock <= DISCOUNT_THRESHOLD"
        type="limited"
        size="sm"
